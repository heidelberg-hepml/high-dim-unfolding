...
N_DISTS = 5  # Number of distributions in the mixture
...
class LengthPredictor(nn.Module):
    ...
    def forward(self, sim_seq, sim_lengths):
        ...
        predictions = torch.stack(
            [head(final_features) for head in self.length_head], dim=-1
        )
...
class ParticleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            "sim_features": torch.FloatTensor(
                self.pad_sequence(self.sim_particles[idx])
            ),
...
def main():
    ...
    print(
        f"Dataset shapes - Sim: {data['sim_particles'].shape}, Gen: {data['gen_particles'].shape}"
    )
    ...
    val_dataset = ParticleDataset(
        val_sim, val_sim_lengths, val_gen_lengths, length_stats=length_stats
    )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i, batch in enumerate(pbar):
            sim_features = batch["sim_features"].to(
        ...
        mean_train_loss = np.mean(
            [loss * ACCUMULATION_STEPS for loss in train_losses]
        )  # Undo accumulation scaling

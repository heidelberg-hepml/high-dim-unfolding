...
from experiments.multiplicity.dataset import QGMultiplicityDataset
...
class MultiplicityExperiment(BaseExperiment):
    """
    Base class for multiplicity experiments, focusing on classification
    """

    def _init_loss(self):
        self.loss = lambda dists, labels: torch.vmap(lambda d, l: -d.log_prob(l))(
            dists, labels
        ).mean()
    ...
    def _evaluate_single(self, loader, title, mode, step=None):
        ...
        with torch.no_grad():
            for batch in loader:
                predicted_distribution, label = self._get_pred_and_label(batch)
                y_pred = predicted_distribution.sample()
        ...
        # bce loss
        metrics["loss"] = self.loss(predicted_distribution, labels_true).item()

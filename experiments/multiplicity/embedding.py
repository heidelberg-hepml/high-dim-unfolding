...
from experiments.multiplicity.dataset import EPS
...
def embed_data_into_ga(fourmomenta, scalars, ptr, cfg_data):
    ...
    ptr: torch.tensor of shape (batchsize+1)
        Indices of the first particle for each event
...
def dense_to_sparse_jet(fourmomenta_dense, scalars_dense):
    ...
    assert (
        fourmomenta_dense.shape[1] == 4
        and scalars_dense.shape[2] == fourmomenta_dense.shape[2]
    )
